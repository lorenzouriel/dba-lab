---
name: dp900-quiz
description: Generate and run DP-900 exam practice Q&A sourced from the lesson content under dp900/content/. Use when the user wants to study, drill, quiz, test themselves, or get practice/review questions for DP-900, or asks to generate flashcards/a study sheet from the dp900 folder. Trigger phrases include "quiz me on DP-900", "dp900 practice questions", "test me on [module]", "generate flashcards from dp900".
---

# DP-900 practice quiz

Generates original exam-style question-and-answer material from the course notes in
`dp900/content/` (relative to the repo root) and drills the user on it.

## Source content

`dp900/content/` is a Microsoft-Learn-style course tree, still being filled in over time:
...
Do not assume a fixed list of domains/modules — glob `dp900/content/**/*.md` (or `Glob`
with pattern `dp900/content/**/*.md`) each time to see what currently exists, since files
get added over time and some modules may still be stubs (an intro-only file).
...
- Mix recall ("what is X") with applied/scenario questions ("a company needs Y, which
  Azure service should they use") and comparison questions ("when should you use A over
  B", e.g. Cosmos DB vs. Azure SQL, batch vs. stream processing).
...
in that case put it under `dp900/practice/` (create the folder if needed), named after
...
- If the user asks for the DP-900 exam's official structure/weighting, don't invent
