...
A BACPAC file is essentially a zipped version of both your database's metadata and data. While this deployment method is compatible with SQL Database, SQL Managed Instance doesn't support migration via BACPAC within the Azure portal. As an alternative, the [SQLPackage utility](/en-us/sql/tools/sqlpackage/sqlpackage?azure-portal=true) should be used with the BACPAC file.

### Bulk Copy Program (BCP)

The [BCP utility](/en-us/sql/tools/bcp-utility?azure-portal=true) is a command-line tool that exports tables to files so you can import them. Use this approach to migrate from a single SQL Database to SQL Managed Instance and back.

### Azure Data Factory (ADF)

[Azure Data Factory](/en-us/azure/data-factory/introduction?azure-portal=true) is built for data movement and orchestration, with the focus on ingestion. ADF has the integration runtime support to run SSIS packages, and the public internet support for SQL Managed Instance.
...
![Diagram showing how connectivity takes place in Azure SQL Managed Instance.](../../wwl-data-ai/migrate-sql-workloads-azure-managed-instances/media/4-connectivity-architecture-example.png)
