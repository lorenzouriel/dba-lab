...
Deploys the shared SSDT projects (`../db/`) through Octopus Deploy using the dacpac workflow: build → package → push → deploy through environments.

## Prerequisites

- **dba-lab running** — SQL Server instances must be up (`../dba-lab.ps1 up` + `init`)
