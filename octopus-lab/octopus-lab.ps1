...
$dbaRoot = Split-Path -Parent $root
...
switch ($Command) {
    ...
    "up" {
        ...
        Write-Host "Make sure dba-lab is running: ..\dba-lab.ps1 up"
        Write-Host ""
        Push-Location $root
        docker compose up -d
        Pop-Location
    ...
    # -- Build dacpac --
    "build" {
        $dbProjectDir = Join-Path $dbaRoot "db"
    ...
    "help" {
        ...
        Write-Host "  ================================================"
        Write-Host ""
        Write-Host "  PREREQUISITE: dba-lab must be running (..\dba-lab.ps1 up + init)"
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "  WORKFLOW"
        Write-Host "    1. ..\dba-lab.ps1 up + init   (start SQL instances)"
