...
```
sqlserver-lab/
├── docker-compose.yml       sql1 + bootstrap; sql2 behind the ha profile
...
```powershell
docker compose up -d sql1           # start
...
## Use case 3 — AI: external model + vector columns (Azure OpenAI)
...
docker compose --profile ha down       # stop everything (volumes survive)
docker compose --profile ha down -v    # full reset — destroys volumes too
```
...
`update-ca-certificates` in a custom sql1 image. `openssl`/`wget` inside the
...
Azure OpenAI (Use case 3) sidesteps the problem entirely.
