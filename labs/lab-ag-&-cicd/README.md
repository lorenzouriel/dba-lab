...
This lab is self-contained under its own folder in the `dba-lab` repo — its `db/`
SSDT project lives here rather than being shared at the repo root:

```
dba-lab/                            # repo root
├── ag-&-cicd-lab/                  # THIS lab
│   ├── docker-compose.yml          # 4 SQL Server instances (mounts ./db as /db)
...
Each lab is self-contained — same AppDB schema (`db/`), same finance-app SSDT
project, copied into each lab that needs it, deployed through a different
pipeline:

```
dba-lab/
├── ag-&-cicd-lab/       # This lab — SQL Server CI/CD with Always On AG
│                          own db/ copy, deployed via msbuild + SqlPackage
│
├── octopus-lab/         # Octopus Deploy CI/CD with dacpac
│                          own db/ copy, deployed via Octopus feed + releases
...
Each lab has its own `README.md`, setup guide (HTML), docker-compose (if applicable), CLI entry point, and `db/` project — labs are independent and don't reach into each other's folders.
