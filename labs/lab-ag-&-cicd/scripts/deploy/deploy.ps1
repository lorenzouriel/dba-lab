...
$labRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$dbProject = Join-Path $labRoot "db"
...
$appDbScript = Join-Path $labRoot "scripts\deploy\output\${Env}_AppDB_${timestamp}.sql"

New-Item -ItemType Directory -Force -Path (Join-Path $labRoot "scripts\deploy\output") | Out-Null
